...
`{fts_indexes, IndexDefinitions}` (defaults to `[]`) configures native
full-text indexes maintained automatically on matching writes. Each
definition is a map with `bucket` (or `bucket_prefix` for tenant-prefixed
buckets), `index`, `columns`, and optional tokenizer settings -- see
docs/FTS.md for the full schema and query API. Definitions are validated at
startup; overlapping definitions sharing an index name fail the start with
`ambiguous_fts_schema`. With the default empty list the store behaves
exactly as a non-FTS Leveled.
